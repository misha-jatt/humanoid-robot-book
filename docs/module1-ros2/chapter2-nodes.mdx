...
import ChapterMetadata from '@site/src/components/ChapterMetadata';
import ExerciseBlock from '@site/src/components/ExerciseBlock';
import CollapsibleSolution from '@site/src/components/CollapsibleSolution';
...
<ExerciseBlock number={1} difficulty="beginner" objective="Review fundamental concepts of ROS 2 nodes">
  <p>Before moving on, test your understanding with these questions:</p>
  <ul>
    <li>What defines a "node" in the context of ROS 2?</li>
    <li>Explain the primary benefits of using multiple small nodes over a single monolithic program in robotics.</li>
    <li>Provide examples of three distinct nodes that might cooperate within a humanoid robot system.</li>
    <li>Why is direct memory sharing between ROS 2 nodes discouraged, and how do they communicate instead?</li>
  </ul>
  <CollapsibleSolution>
    <p>Consider the following points for your answers:</p>
    <ul>
      <li>A **node** is an independent, executable program within ROS 2 that performs a specific, single task (e.g., controlling a motor, processing camera input).</li>
      <li>**Benefits of multiple nodes**: Modularity (clearer code), easier development/testing (isolated units), fault tolerance (one crash doesn't halt the system), hardware independence (nodes can run on different physical machines), and reusability (nodes can be used in various projects).</li>
      <li>**Examples**: A Camera Node (captures images), a Vision Node (processes images), and a Leg Control Node (moves legs).</li>
      <li>**No direct memory sharing**: Nodes are isolated to prevent one node's errors from corrupting another's data. They communicate exclusively by sending **messages** (e.g., via topics), ensuring robust and clear interactions.</li>
    </ul>
  </CollapsibleSolution>
</ExerciseBlock>
