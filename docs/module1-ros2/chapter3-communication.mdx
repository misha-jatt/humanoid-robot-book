...
```mermaid
flowchart LR
    subgraph Topics["Topics (Streaming)"]
        P1[Publisher] -->|continuous| T1((Topic))
        T1 --> S1[Subscriber 1]
        T1 --> S2[Subscriber 2]
    end

    subgraph Services["Services (Query)"]
        C[Client] -->|request| SRV((Service))
        SRV -->|response| C
    end

    subgraph Actions["Actions (Tasks)"]
...
*Figure 1: The three ROS 2 communication patterns*
